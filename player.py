...
def handle_exception(exc_type, exc_value, exc_traceback):
    ...
    if(not debug.debugEnabled() and exc_type != KeyboardInterrupt):
...
debug.debugPrint("Global variables initialized.\nInitializing Pygame...")

pygame.init()

debug.debugPrint("Pygame initialized. Setting up window...")
...
debug.debugPrint("Window set up.\nReading from preferences.ini....")
...
try:
    toontuber.animationSFXVolume = float(prefini["Settings"]["volume"])
except Exception as e:
    ...
    toontuber.animationSFXVolume = 0.5
...
prefini["Settings"] = {"settingskey": f"\"{settingsKeybindName}\"", "bgcolor": BGCOLOR, "antialiasing": int(antialiasing), "ignorehotkey": f"\"{ignoreHotkeyBindName}\"", "mutekey": f"\"{muteKeyName}\"", "volume": toontuber.animationSFXVolume, "sfxmutekey": f"\"{sfxMuteKeyName}\"", "mirror": f"{'1' if mirror else '0'}", "suppression": f"{'1' if suppression else '0'}"}
...
debug.debugPrint("preferences.ini read.\nInitializing audio data stuff...")
...
debug.debugPrint("Audio data stuff initialized.\nInitializing input reader thread...")
...
def pushHotKey(key):
    ...
    if(key in hotkeyDictionary):
        ...
        debug.debugPrint(f"After pressing {key}, the possible animations are {possibleAnims}\nRemoving the current and queued animations if they're in there...")
        ...
        debug.debugPrint(f"After removing the current and queued animations, the possible animations are {possibleAnims}\nRemoving any animations that are blocked by the current animation...")
        ...
        debug.debugPrint(f"After removing blocked animations, the possible animations are {possibleAnims}. Getting only the ones where the current animation is a requirement...")
        ...
        if(len(possibleAnims) == 0):
            debug.debugPrint("No animations left to choose from. Returning.")
            return
        ...
        debug.debugPrint(f"Randomly selecting an animation from the remaining list of {possibleAnims} animations...")
        resultingAnim = random.choices(possibleAnims, requirementWeights)[0]
        debug.debugPrint(f"Resulting animation: {resultingAnim}")
...
def streamdeck_key_press_callback(deck, key, state):
    if state:
        debug.debugPrint(f"Stream Deck Key {key} was pressed")
        pushHotKey(f"SD_{deck.id()}_{key}")
        # format for StreamDeck hotkeys: SD_<deck id>_<key number>

debug.debugPrint("Checking for connected StreamDecks...")
...
try:
    ...
    debug.debugPrint(f"({len(streamdecks)}) StreamDeck found.")
except Exception as e:
    debug.debugPrint("NO StreamDecks were found.")

# keyboard reading thread
allKeysPressed = []

def keyboard_input_thread():
    global currentScreen, muteKeyName, settingsKeybindName, ignoreHotkeyBindName, changingAnyKeybind, ignoreHotkey, muted, settingsText
    while True:
        ...
        if event.event_type == "down":
            ...
            if(event.name == ignoreHotkeyBindName):
                ...
                toontuber.animationSFXMuted = not toontuber.animationSFXMuted
                settingsText[6].text = f"Animation SFX Vol: {round(toontuber.animationSFXVolume*100) if not toontuber.animationSFXMuted else '(MUTED)'}"
            else:
...
debug.debugPrint("Input reader thread started.\nCreating GUI classes and button actions...")
        ...
        if image_timer > 1 / framerate: # time equivelent to one frame has passed
            ...
            # otherwise, check if we aren't locked in the current animation and if it has finished
                ...
                if((queuedExpression != currentExpression and queuedExpression != "") or transition != "" or (currentAnimationType == "canned")):
                    debug.debugPrint("animation is finished and new animation is queued.")
                        ...
                        if(transition == ""):
                                ...
                                if(currentAnimationType == "canned"):
                                    ...
                                    if(expressionList[expressionIndex[currentExpression]].getTransitionIn().exists()):
                                        ...
                                        if(expressionList[expressionIndex[currentExpression]].getSound() != None):
                                            toontuber.playAnimationSound(expressionList[expressionIndex[currentExpression]].getSound())
                                    else:
                                        transition = ""
                                        loadExpression(expressionList[expressionIndex[currentExpression]], "main")
                                        if(expressionList[expressionIndex[currentExpression]].getSound() != None):
                            ...
                            if(currentAnimationType == "expression"):
                                ...
                                if(expressionList[expressionIndex[currentExpression]].getTransitionIn().exists()):
                                    transition = "in"
                                    loadExpression(expressionList[expressionIndex[currentExpression]], "in")
                                    debug.debugPrint(expressionList[expressionIndex[currentExpression]].getSound())
                                    if(expressionList[expressionIndex[currentExpression]].getSound() != None):
                                        toontuber.playAnimationSound(expressionList[expressionIndex[currentExpression]].getSound())
                                else:
                                    transition = ""
                    ...
                    # queued expression and transitions
                        ...
                        debug.debugPrint("Canned animation completed. Loading resulting expression...")
...
debug.debugPrint("GUI classes created.\nCreating Tuber loading functions...")
...
def loadExpression(expressionSet, animation):
    ...
    if(animation == "main" and expressionSet.getMain().exists()):
        ...
        print(f"toontuber.ExpressionSet \"{expressionSet.getName()}\" does not have \"{animation}\" animation.")
        return
    currentAnimationType = "expression"
    debug.debugPrint(f"Loading animation \"{animation}{f' {toontuber.idleChoiceIndex}' if animation == 'idle' else ''}\" from expression set \"{expressionSet.getName()}.\"")
...
def loadCanned(cannedAnimation):
    ...
    debug.debugPrint(f"Loading CANNED animation \"{cannedAnimation.getName()}.\"")
    idling = False
    idle_timer_reset()
    randIdleMin = -1
    randIdleMax = -1
    ...
    if(cannedAnimation.getSound() != None):
        toontuber.playAnimationSound(cannedAnimation.getSound())
...
def loadTuber(path):
    ...
    created = ""
    modified = ""
    toontuber.randomDuplicateReduction = 0
    expressionList = []
    cannedAnimationList = []
    hotkeyDictionary = {}
    expressionIndex = {}
    cannedAnimationIndex = {}
    toontuber.idleChoiceIndex = -1
    ...
    debug.debugPrint(f"Loading {data['name']}...")
    # print(data)
    ...
    toontuber.randomDuplicateReduction = data["random_duplicate_reduction"]
    ...
    name1 = ""
    debug.debugPrint(f"Loading {expressionCount} expressions and {cannedCount} canned animations...")
    expDone = 0
    for expressionData in data["expressions"]:
        ...
        main = toontuber.Animation(expressionData["anims"]["Main"]["frames"], expressionData["anims"]["Main"]["fps"], expressionData["anims"]["Main"]["locking"])
        debug.debugPrint("main animation loaded. loading idles...")

        # idle set
        idleList = []
        if(not expressionData["anims"]["Idles"]):
            # print("No idles found.")
            idles = toontuber.IdleSet(None, None, None)
            randIdleMax = -1
            randIdleMin = -1
        else:
            for idle in expressionData["anims"]["Idles"]["idleAnims"]:
                idleList.append(toontuber.Animation(idle["frames"], idle["fps"], idle["locking"]))
            idles = toontuber.IdleSet(idleList, expressionData["anims"]["Idles"]["randomSecMin"], expressionData["anims"]["Idles"]["randomSecMax"])
        # print(f"all idles for {expressionData['name']}:\n {idleList}")
        debug.debugPrint("idles loaded. loading talking...")

        # talking
        if(not expressionData["anims"]["Talk"]):
            # print("No talking animation found.")
            talking = toontuber.Animation(None, None, None)
        else:
            talking = toontuber.Animation(expressionData["anims"]["Talk"]["frames"], expressionData["anims"]["Talk"]["fps"], expressionData["anims"]["Talk"]["locking"])

        debug.debugPrint("talking loaded. loading peak...")
        # peak
        if(not expressionData["anims"]["Peak"]):
            # print("No peak animation found.")
            peak = toontuber.Animation(None, None, None)
        else:
            peak = toontuber.Animation(expressionData["anims"]["Peak"]["frames"], expressionData["anims"]["Peak"]["fps"], expressionData["anims"]["Peak"]["locking"])

        debug.debugPrint("peak loaded. loading transition in...")

        # transition in
        if(not expressionData["anims"]["TransitionIN"]):
            # print("No transition in animation found.")
            transitionIn = toontuber.Animation(None, None, None)
        else:
            transitionIn = toontuber.Animation(expressionData["anims"]["TransitionIN"]["frames"], expressionData["anims"]["TransitionIN"]["fps"], True)

        debug.debugPrint("transition in loaded. loading transition out...")

        # transition out
        if(not expressionData["anims"]["TransitionOUT"]):
            # print("No transition out animation found.")
            transitionOut = toontuber.Animation(None, None, None)
        else:
            transitionOut = toontuber.Animation(expressionData["anims"]["TransitionOUT"]["frames"], expressionData["anims"]["TransitionOUT"]["fps"], True)

        debug.debugPrint("transition out loaded. creating expression set...")
        # create toontuber.ExpressionSet
        expressionList.append(toontuber.ExpressionSet(expressionData["name"], main, idles, talking, peak, transitionIn, transitionOut, expressionData["requires"], expressionData["blockers"], expressionData['sound'], expressionData['instant'], expressionData["hotkeys"]))
        expressionIndex[expressionData["name"]] = len(expressionList) - 1

        debug.debugPrint("expression set created. adding to hotkey dictionary...")
    ...
    debug.debugPrint("all expressions loaded. loading canned animations...")
    canDone = 0
    ...
    debug.debugPrint("all canned animations loaded. loading expressions...")
    # print(hotkeyDictionary)
...
def loadTuberThread(path=None):
    if(path is None):
        path = file_reading.selectJSON()
    else:
        file_reading.setJSONPath(path)
...
def toggleDebug():
    ...
    if(debug.debugEnabled()):
        debug.debugPrint("=== Debug mode disabled. ===")
    debug.setDebug(not debug.debugEnabled())
    if(debug.debugEnabled()):
        debug.debugPrint("=== Debug mode ENABLED. ===")
    settingsText[7].setText(f"{'DEBUG MODE ENABLED' if debug.debugEnabled() else 'Debug Mode disabled'}")
...
settingsText = [
    ...
    ClickableText(f"Animation SFX Vol: {round(toontuber.animationSFXVolume*100) if not toontuber.animationSFXMuted else '(MUTED)'}", (0, height-175), UniFontSmaller, WHITE, None),
    ClickableText(f"{'DEBUG MODE ENABLED' if debug.debugEnabled() else 'Debug Mode disabled'}", (0, 100), UniFontSmaller, WHITE, toggleDebug),
]
...
debug.debugPrint("Tuber loading functions created.\nSetting up final GUI elements and functions...")
...
volumeSlider = pygame_gui.elements.UIHorizontalSlider(relative_rect=pygame.Rect((0, height - 150), (150, 25)),
                                                                          start_value=toontuber.animationSFXVolume,
                                                                          value_range=(0, 1),
                                                                          manager=settings_UImanager)
...
debug.debugPrint("Creating mic meter sprites...")
...
while running:
    ...
    for event in pygame.event.get():
        ...
        # if the user clicks on a text option, do the action
            ...
            toontuber.animationSFXVolume = volumeSlider.get_current_value()
            settingsText[6].text = f"Animation SFX Vol: {round(toontuber.animationSFXVolume*100) if not toontuber.animationSFXMuted else '(MUTED)'}"
            prefini.set("Settings", "volume", str(toontuber.animationSFXVolume))
            with open("preferences.ini", "w") as ini:
                prefini.write(ini)
    ...
    if currentScreen == "opening":
        ...
        draw_text_options([ClickableText("Loading tuber, please wait...", (50, 50), UniFont, WHITE, None),
                           ...
                           ClickableText(file_reading.currentlyLoadingFile, (50, 250), UniFont, WHITE, None)])
